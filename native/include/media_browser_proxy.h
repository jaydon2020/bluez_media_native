...
class MediaBrowserProxy {
 public:
  ...
  static std::vector<uint8_t> encode_folder_properties(
      const std::string& folder_path,
      const std::map<std::string, sdbus::Variant>& properties);
  static std::vector<uint8_t> encode_item_properties(
 ...
 private:
  ...
  std::unique_ptr<sdbus::IProxy> make_folder_proxy(
      const std::string& folder_path) const;
  std::unique_ptr<sdbus::IProxy> make_item_proxy(
      const std::string& item_path) const;

  static BlueZMediaItemProps item_from_properties(
