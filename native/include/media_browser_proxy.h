...
public:
  ...
  std::vector<uint8_t> folder_search(const std::string &folder_path, const std::string &value) const;
  std::vector<uint8_t> folder_list_items(const std::string &folder_path) const;
  int folder_change_folder(const std::string &folder_path, const std::string &target_folder_path) const;
...
private:
  ...
  std::unique_ptr<sdbus::IProxy> make_folder_proxy(const std::string &folder_path) const;
  std::unique_ptr<sdbus::IProxy> make_item_proxy(const std::string &item_path) const;
  
  static BlueZMediaItemProps item_from_properties(const std::string &object_path,
