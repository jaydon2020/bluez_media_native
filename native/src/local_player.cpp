...
LocalPlayer::LocalPlayer(sdbus::IConnection& conn,
  ...
  media_call(*media_proxy, kMediaIface, "RegisterPlayer",
             sdbus::ObjectPath{state_.player_path}, properties);
}
...
void LocalPlayer::register_mpris_object() {
  ...
  const auto unsupported = [] {
    throw sdbus::Error{
        sdbus::Error::Name{"org.freedesktop.DBus.Error.NotSupported"},
  ...
  object_
      ->addVTable(
          ...
          sdbus::registerMethod("Seek").implementedAs(
              [unsupported](int64_t) { unsupported(); }),
          sdbus::registerMethod("SetPosition")
              .implementedAs([unsupported](const sdbus::ObjectPath&, int64_t) {
                unsupported();
              }),
...
LocalPlayer::~LocalPlayer() {
  ...
  try {
    auto media_proxy =
        sdbus::createProxy(conn_, sdbus::ServiceName{kBluezService},
                           sdbus::ObjectPath{state_.adapter_path});
    media_call(*media_proxy, kMediaIface, "UnregisterPlayer",
               sdbus::ObjectPath{state_.player_path});
...
std::map<std::string, sdbus::Variant> LocalPlayer::make_player_properties()
  ...
  properties["Metadata"] =
      sdbus::Variant{std::map<std::string, sdbus::Variant>{}};
  properties["LoopStatus"] = sdbus::Variant{loop_status_};
  properties["Shuffle"] = sdbus::Variant{shuffle_};
  for (const auto* capability : {"CanPlay", "CanPause", "CanSeek", "CanGoNext",
