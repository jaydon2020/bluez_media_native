...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        BluezMediaClient.create(coverArtMode: BluezMediaCoverArtMode.disabled),
        throwsA(
  ...
  test('disabled client never creates a cover-art session', () async {
    await step('owned_thumbnail');
    final client = await BluezMediaClient.create(
      coverArtMode: BluezMediaCoverArtMode.disabled,
    );
    try {
      expect(
        () => client.getPlayerCoverArt('/player', '/tmp/disabled-cover-art'),
        throwsStateError,
      );
      await Future<void>.delayed(const Duration(milliseconds: 100));
